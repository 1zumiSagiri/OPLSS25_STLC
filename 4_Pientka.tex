...
\begin{minipage}{3in}
    \begin{mathpar}
        \infer*{\Delta; \Gamma \vdash M: \Box(\Psi \Vdash A) \; \; \; \; (\Delta, u:(\Psi \Vdash A)); \Gamma \vdash N}{\Delta; \Gamma \vdash \textsf{let } \BOX u = M \textsf{ in } N:C} \notate{(\Box E)}
    \end{mathpar}
\end{minipage}
...
\begin{minipage}{3in}
    \begin{mathpar}
        \infer*{x: A \textsf{ true} \in \Gamma}{\Delta; \Gamma \vdash x: A}
    \end{mathpar}
\end{minipage}
\begin{minipage}{3in}
\begin{mathpar}
    \infer*{u: (\Psi \Vdash A) \in \Delta \; \; \; \; \Delta; \Gamma \vdash \sigma: \Psi}{\Delta; \Gamma \vdash \textsf{clo}(u, \sigma):A}
\end{mathpar}
\end{minipage}
